const Success = ({ successType, email }) => {
  function maskEmail(string) {
    let [localPart, domain] = string.split('@')

    let maskedLocalPart = localPart[0] + '*'.repeat(localPart.length - 1)
    let [domainName, domainTLD] = domain.split('.')
    let maskedDomainName = domainName[0] + '*'.repeat(domainName.length - 1)

    let maskedEmail = `${maskedLocalPart}@${maskedDomainName}.${domainTLD}`
    ...
    <div className='w-full h-full flex flex-col justify-center items-center text-center container mx-auto'>
      ...
      <div className='my-10'>
        ...
        {successType === 'create' ? (
          ...
          <p className='mt-3'>
            <span className='capitalize'>we </span>just emailed instructions and a reset password link to <span className='font-semibold'>{maskEmail(email)}. </span>
            <br className='hidden md:inline' />
